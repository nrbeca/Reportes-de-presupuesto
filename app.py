            ...
            with tab2:
                ...
                if not resultados_ur:
                    ...
                    with col_der:
                        ...
                        def colorear_fila_cap(row):
                            if row['Capitulo'] == 'Total':
                                return [f'background-color: {COLOR_BEIGE}; font-weight: bold'] * len(row)
                            else:
                                return [f'background-color: {COLOR_GRIS}'] * len(row)
                        
                        styled_cap = df_cap.style.apply(colorear_fila_cap, axis=1).format({
