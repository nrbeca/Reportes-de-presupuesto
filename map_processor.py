...
def procesar_map(df, filename):
    ...
    # Calcular congelados por programa específico
    ...
    for prog in programas_con_congelados:
        df_prog = df[df['PROGRAMA'] == prog]
        if len(df_prog) > 0:
            cong_prog = round_like_excel(df_prog['CONG_ANUAL'].sum(), 2) if 'CONG_ANUAL' in df_prog.columns else 0
            congelados_valores[prog] = cong_prog
            congelados_textos[prog] = numero_a_letras_mx(cong_prog)
        else:
            congelados_valores[prog] = 0
            congelados_textos[prog] = numero_a_letras_mx(0)
    
    congelado_anual_total = df['CONG_ANUAL'].sum() if 'CONG_ANUAL' in df.columns else 0
    congelado_periodo_total = df['CONG_PERIODO'].sum() if 'CONG_PERIODO' in df.columns else 0

    return {
        'congelados': {
            'valores': congelados_valores,
            'textos': congelados_textos,
            'anual': congelado_anual_total,
            'periodo': congelado_periodo_total,
        },
        'totales': totales,
